#!/bin/bash
# MT5 consumer watchdog — runs every 5 min via systemd timer.
#
...
#
#   - Ensures mt5-tunnel and mt5-consumer are active (restarts if failed).
#   - Only enforces consumer *liveness* when the forex market is OPEN, because
#     the consumer intentionally idles (and stays quiet) when the market closes.
    ...
    if [ -n "$TG_TOKEN" ] && [ -n "$TG_CHATS" ]; then
        ...
        for chat in "${CHATS[@]}"; do
            curl -s -m 10 -X POST "https://api.telegram.org/bot${TG_TOKEN}/sendMessage" \
                -d chat_id="${chat}" --data-urlencode text="[MT5 consumer] ${msg}" >/dev/null 2>&1 || true
        done
    fi
}
...
# ── 1) Tunnel + consumer units must be active ────────────────
ensure_active mt5-tunnel
ensure_active mt5-consumer

# ── 2) Consumer liveness — only while the market is open ─────
if $PY -c "from app.utils import forex_hours; import sys; sys.exit(0 if forex_hours.is_market_open() else 1)" 2>/dev/null; then
    LAST=$(journalctl -u mt5-consumer -n 1 --output=short-unix --no-pager 2>/dev/null | tail -1 | awk '{print int($1)}')
    NOW=$(date +%s)
    if [ -z "$LAST" ] || [ "$LAST" -eq 0 ]; then
        alert "no logs from mt5-consumer while market OPEN — restarting"
        systemctl restart mt5-consumer
    else
        SILENCE=$((NOW - LAST))
        if [ "$SILENCE" -gt "$MAX_SILENCE" ]; then
            alert "mt5-consumer silent ${SILENCE}s (>${MAX_SILENCE}s) while market OPEN — restarting"
            systemctl restart mt5-consumer
        else
            echo "[$(date)] OK: mt5-consumer last log ${SILENCE}s ago (market open)"
        fi
    fi
else
